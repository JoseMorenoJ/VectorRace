...
Circuit::Circuit(char *file_path)
{
    ...
    std::cout << "circuit(file_path)" << std::endl;
    read_bmp(file_path);
    init_grid();
}
...
bool Circuit::is_inside(Coordinate coord)
{
    // Screen limits
...
std::vector<GridElement> Circuit::read_bmp(char *filename)
{
    std::fstream fich;
    fich.open(filename);
